...
class UserTableProceduresTest {
    ...
    void testGettingUserByUsername() throws Exception {
        ...
        try {
            User selectedUser = userTableProcedures.findByUsername(USERNAME);
    ...
    void testGettingUserById() throws Exception {
        ...
        try {
            User selectedUser = userTableProcedures.findById(ID);
    ...
    void testLookingForUserThatDoesNotExist() {
        ...
        assertThrows(EntryNotFoundException.class,
                () -> userTableProcedures.findByUsername(SECOND_USERNAME));

        assertThrows(EntryNotFoundException.class,
                () -> userTableProcedures.findById(SECOND_ID));
    }
    ...
    * be better. ChangeUserLoginStatus and aUserIsLoggedIn seem to be strongly coupled.*/
    ...
    void testChangingLoginStatus() throws Exception {
        ...
        User testUser = userTableProcedures.findByUsername(USERNAME);
        testUser.setIsLoggedIn(true);
        userTableProcedures.changeUserLoginStatus(testUser);

        User newTestUserModel = userTableProcedures.findByUsername(USERNAME);
        Assertions.assertTrue(newTestUserModel.getIsLoggedIn());

        testUser.setIsLoggedIn(false);
        userTableProcedures.changeUserLoginStatus(testUser);

        User anotherTestUserModel = userTableProcedures.findByUsername(USERNAME);
        Assertions.assertFalse(anotherTestUserModel.getIsLoggedIn());
    }

    @Test
    void testAUserIsLoggedIn() throws Exception {
    ...
    @Test
    void testThatGetIsLoggedInThrowsNotFoundException() throws Exception {

        testUser.logout();
        assertThrows(EntryNotFoundException.class, () -> userTableProcedures.findLoggedInUser());
    }
    ...
    void testThatLoggedInUserIsFound() throws Exception {
        ...
        testUser.login();
        User loggedInUser = userTableProcedures.findLoggedInUser();
    ...
    void testAddingNewUser() throws Exception {
        ...
        userTableProcedures.addEntryToDatabase(incompleteUserModel);
    ...
    void testDeletingUser() throws Exception {
        ...
        User incompleteUserModel = TestService.createTestUser(SECOND_USERNAME);
        userTableProcedures.addEntryToDatabase(incompleteUserModel);
        userTableProcedures.deleteUser(SECOND_USERNAME);
    ...
    @Test
    void testTakenUsernameThrowsException() throws Exception {

        User duplicateUser = TestService.createTestUser(USERNAME);
        assertThrows(SQLException.class,
                () -> userTableProcedures.addEntryToDatabase(duplicateUser));
    }

    @Test
    void testChangingUserBalance() throws Exception {

        testUser.setBalance(THREE_HUNDRED);
        userTableProcedures.changeUserBalance(testUser);

        Assertions.assertEquals(THREE_HUNDRED, userTableProcedures.findByUsername(USERNAME).getBalance());
    }
}